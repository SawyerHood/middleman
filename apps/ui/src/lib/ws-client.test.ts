...
describe("ManagerWsClient", () => {
  ...
  it("sends update_manager_model and resolves with manager_updated event", async () => {
    const client = new ManagerWsClient("ws://127.0.0.1:8787", "manager");
    ...
    const updatePromise = client.updateManagerModel("manager", "pi-opus");

    const sentUpdatePayload = JSON.parse(socket.sentPayloads.at(-1) ?? "{}");
    expect(sentUpdatePayload).toMatchObject({
      type: "update_manager_model",
      managerId: "manager",
      model: "pi-opus",
    });
    expect(typeof sentUpdatePayload.requestId).toBe("string");

    emitServerEvent(socket, {
      type: "manager_updated",
      requestId: sentUpdatePayload.requestId,
      manager: {
